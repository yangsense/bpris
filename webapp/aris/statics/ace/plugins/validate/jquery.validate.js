    ...
    $.extend($.validator, {
        ...
        prototype: {
            ...
            elements: function() {
				...
				return $([]).add(listeElements).filter(":input").not(":submit, :reset, :image, [disabled]").not(this.settings.ignore).filter(function() { ! this.name && validator.settings.debug && window.console && console.error("%o has no name assigned", this);
...
(function($) {
    $.validator.addMethod("leDate",
    function(value, element, param) {
        if (value == "") return true;
        return new Date(Date.parse($(param).val().replace("-", "/"))) <= new Date(Date.parse(value.replace("-", "/")));
    },
    "结束日期必须大于开始日期!");

    $.validator.addMethod("geDate",
    function(value, element, param) {
        if (value == "") return true;
        return new Date(Date.parse($(param).val().replace("-", "/"))) >= new Date(Date.parse(value.replace("-", "/")));
    },
