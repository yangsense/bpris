...
var setting = {
    ...
    async: {
        ...
        enable: true,
        url: GLOBAL.WEBROOT + "/studyReport/initTree.ajax?orgId=" + $("#orgId", parent.parent.document).val() + "&flag=" + 'public' + "&locId=" + parent.$("#locId").val() + "&modalityId=" + parent.$("#modalityId").val(),
        autoParam: ["id"]
    },
    callback: {
...
//		treeNode = zTree.addNodes(treeNode, {id:nodeId, pId:treeNode.id, isParent:isParent, name:"new node" + (newCount++)});
        ...
        treeNode = zTree.addNodes(null, {id: nodeId, pId: 0, isParent: isParent, name: "模板"});
        layer.msg("叶子节点被锁定，无法增加子节点");
    }
