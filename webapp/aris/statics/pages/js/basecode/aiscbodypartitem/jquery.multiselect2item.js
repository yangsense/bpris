	...
	var methods = {
		init : function(options) {
			...
			return this.each(function () {
				...
				var htmlToAdd = 
					"<div class='ms2side__div1'>" +
							((o.selectedPosition != 'right' && o.moveOptions) ? divUpDown : "") +
						"<div class='ms2side__select1'>" +
							((o.labelsx || leftSearch != false) ? ("<div class='ms2side__header1'>" + (leftSearch != false ? leftSearch : o.labelsx) + "</div>") : "") +
							"<select title='" + o.labelsx + "' name='" + nameSx + "' id='" + nameSx + "' size='" + size + "' multiple='multiple' ></select>" +
						"</div>" +
						"<div class='ms2side__options1'>" +
						...
						"</div>" +
						"<div class='ms2side__select1'>" +
							((o.labeldx || rightSearch != false) ? ("<div class='ms2side__header1'>" + (rightSearch != false ? rightSearch : o.labeldx) + "</div>") : "") +
				...
				// ELEMENTS
				var allSel = el.next().children(".ms2side__select1").children("select");
				...
				// HEIGHT DIV
				var	heightDiv = $(".ms2side__select1").eq(0).height();
				...
				$(this).next().find('.ms2side__options1, .ms2side__updown1').each(function(){
				...
				allSel.change(function() {
					...
					if (selectedSx.size() == 0 ||
						...
						div.find(".AddOne").addClass('ms2side__hide1');
					else
						div.find(".AddOne").removeClass('ms2side__hide1');

					// FIRST HIDE ALL
					div.find(".RemoveOne, .MoveUp, .MoveDown, .MoveTop, .MoveBottom, .SelSort").addClass('ms2side__hide1');
					if (selectDx.size() > 1)
						div.find(".SelSort").removeClass('ms2side__hide1');
					if (selectedDx.size() > 0) {
						div.find(".RemoveOne").removeClass('ms2side__hide1');
						...
						if (selectedDx.size() < selectDx.size()) {	// FOR NOW (JOE) && selectedDx.size() == 1
							if (selectedDx.val() != selectDx.val())	// FIRST OPTION, NO UP AND TOP BUTTON
								div.find(".MoveUp, .MoveTop").removeClass('ms2side__hide1');
							if (selectedDx.last().val() != selectDx.last().val())	// LAST OPTION, NO DOWN AND BOTTOM BUTTON
								div.find(".MoveDown, .MoveBottom").removeClass('ms2side__hide1');
						}
					}

					if (selectSx.size() == 0 ||
							(o.maxSelected >= 0 && selectSx.size() >= o.maxSelected))
						div.find(".AddAll").addClass('ms2side__hide1');
					else
						div.find(".AddAll").removeClass('ms2side__hide1');

					if (selectDx.size() == 0)
						div.find(".RemoveAll").addClass('ms2side__hide1');
					else
						div.find(".RemoveAll").removeClass('ms2side__hide1');
				});
				...
				// CLICK ON OPTION
				$(this).next().find('.ms2side__options1').children().click(function () {
					if (!$(this).hasClass("ms2side__hide1")) {
				...
				// CLICK ON UP - DOWN
				$(this).next().find('.ms2side__updown1').children().click(function () {
					var	selectedDx = rightSel.find("option:selected");
					var	selectDx = rightSel.find("option");
				...
				// HOVER ON OPTION
				$(this).next().find('.ms2side__options1, .ms2side__updown1').children().hover(
					function () {
						$(this).addClass('ms2side_hover1');
					},
					function () {
						$(this).removeClass('ms2side_hover1');
					}
				);
		...
		destroy : function( ) {
			return this.each(function () {
				var	el = $(this);
				var data = el.data('multiselect2side1');
