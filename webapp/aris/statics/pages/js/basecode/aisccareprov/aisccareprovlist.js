...
function selectList() {
	...
	$(grid_selector).jqGrid({
        url: GLOBAL.WEBROOT +"/basecode/queryAiscCareProvList.ajax",
        mtype : "post",  
        datatype: "json",
        height: '100%',
        width : '100%',
        postData :data,
        colNames: ['人员ID','人员代码','人员名称','人员类别','所属科室','机构名称','操作'],
        colModel: [
            {name: 'careprovId', index: 'careprovId', width:60,sortable: false},
            {name: 'careprovCode', index: 'careprovCode', sortable: false},
            {name: 'careprovName', index: 'careprovName', sortable: false},
            {name: 'careprovTypeDesc', index: "careprovTypeDesc", sortable: false},
...
function view(id,type) {
	...
	if(type=="query"){
		title="查看人员信息";
		url="/basecode/aiscCareProvDetail.html?id="+id+"&type="+type;
		//所有控件不可用
	}else if(type=="update"){
		title = "修改人员信息";
		url="/basecode/aiscCareProvDetail.html?id="+id+"&type="+type;
...
function deleData(param){
	layer.confirm('是否确定删除该信息？', {
        ...
        type: "GET",
        async: true,
        url: GLOBAL.WEBROOT +"/basecode/deleteAiscCareProv.ajax?id="+param,
        dataType: 'json',
        success: function (data) {
			if (data.ERRCODE == "0") {
