...
function initDict() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var loc_TypeSelect = $("#careprovTypeid");
            loc_TypeSelect.empty();
            loc_TypeSelect.append("<option value='-1'>请选择类别</option>");
            for (var i = 0; i < jsonAiscCareProvType.length; i++) {
                loc_TypeSelect.append("<option value='" + jsonAiscCareProvType[i].itemno + "'>" + jsonAiscCareProvType[i].itemname + "</option>");
            }
...
function reloadGrid(){
	...
	$("#aiscordcat2loclist").jqGrid('setGridParam', {postData: data,page:1,pageSize:10}).trigger("reloadGrid");
}

//查询列表
function selectList() {
	var grid_selector = "#aiscordcat2loclist";
    var pager_selector = "#aiscordcat2loc_pager";
    var data = $("#searchForm").serializeObject();
	$(grid_selector).jqGrid({
        url: GLOBAL.WEBROOT +"/basecode/queryAiscOrdCat2LocList.ajax",
        mtype : "post",  
        datatype: "json",
        height: '100%',
        width : '100%',
        postData :data,
        colNames: ['科室名称','检查大类名称','组织名称','操作'],
        colModel: [
            {name: 'locDesc', index: 'locDesc',width:100, sortable: false},
            {name: 'ordcategoryDesc', index: 'ordcategoryDesc', sortable: false},
            {name: 'orgName', index: "orgName", sortable: false},
            {name: 'ordcat2locId', index: "ordcat2locId", width:60, sortable: false
...
function view(id,type) {
	...
	if(type=="query"){
		title="查看科室检查大类信息";
		url="/basecode/viewDetail.html?id="+id+"&type="+type;
		//所有控件不可用
	}else if(type=="update"){
		title = "修改科室检查大类信息";
		url="/basecode/viewDetail.html?id="+id+"&type="+type;
...
function deleData(param){
	layer.confirm('是否确定删除该信息？', {
        ...
        type: "GET",
        async: true,
        url: GLOBAL.WEBROOT +"/basecode/deleteAiscOrdCat2Loc.ajax?id="+param,
        dataType: 'json',
        success: function (data) {
			if (data.ERRCODE == "0") {
