...
(function($) {
	$.addFlex = function(t, p) {
		...
		var g = {
			...
			buildHeader : function() {
				...
				if (p.showRowNum) {
					$(tr)
							.append("<th width='60' axis='colrownum' adduser=true align='center' id='gridRowNumColumn'>行号</th>");
				}
		...
		if (p.buttons) {
			...
			for (var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					if(!btn.sepstate || btn.sepstate=='inline'){
	...
	$.fn.flexResize = function(h, w) {
		return this.each(function() {
					...
					if (h > this.p.minheight) {
						this.grid.bDiv.style.height = (h-14) + 'px'
						this.p.height = h-14;
					}
				});
	}
	...
	$.fn.flexSearch = function(params) {
		return this.each(function() {
					...
					// 修改，默认调用该方法的时候，翻页到第一页；  先屏蔽掉此模式，如果有问题在恢复   update by  hanlq
					this.p.newp = 1;
